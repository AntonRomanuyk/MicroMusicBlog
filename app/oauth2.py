...
async def get_current_user(token: str = Depends(oauth2_scheme), db: AsyncSession = Depends(database.get_db)) -> schemas.UserOut:
    ...
    user_dict = await cache.fetch_with_stampede_protection(
        cache_key, fetch_user_from_db, expire=settings.cache_TTL
    )
