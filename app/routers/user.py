...
# NOTE: Unbounded query. Pagination is not implemented by design for simplicity.
# To be refactored when the dataset grows significantly.
@router.get("/all", response_model=list[schemas.UserOut])
async def get_all_users(db: AsyncSession = Depends(get_db)):
    cache_key = "users:all"

    async def fetch_data_from_db():
        query = select(models.User).options(joinedload(models.User.avatar)).filter(models.User.is_deleted.is_(False))
