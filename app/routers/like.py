...
from app import models, oauth2, schemas, cache
...
def like_post(id: int, db: Session = Depends(get_db), current_user: schemas.UserOut = Depends(oauth2.get_current_user)):
    try:
        ...
        stmt = select(models.post_likes).where(models.post_likes.c.post_id == id,
...
def unlike_post(id: int, db: Session = Depends(get_db), current_user: schemas.UserOut = Depends(oauth2.get_current_user)):
    try:
        post = db.get(models.Post, id)
        if post is None:
            raise HTTPException(status_code=HTTP_404_NOT_FOUND, detail=f"Post with id: {id} not found")
