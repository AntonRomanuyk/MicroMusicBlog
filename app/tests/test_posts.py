...
async def test_create_post_with_files(
    ...
    file_result = await db_session.execute(select(models.PostFile).filter(models.PostFile.post_id == post.id))
...
async def test_delete_post_cascade(
    ...
    comment_result = await db_session.execute(select(models.Comment).filter(models.Comment.post_id == post.id))
